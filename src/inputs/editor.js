    ...
    return (editor, _completion, from, to) => {
        ...
        if (ranges.length > 1) {
            ...
            if (editor.state.field(snippetState, false) === undefined)
                effects.push(StateEffect.appendConfig.of([snippetState, addSnippetKeymap, snippetPointerHandler, baseTheme$7]));
        }
...
var setup = [lineNumbers() , highlightActiveLineGutter(), highlightSpecialChars(), history(), // languages(),
javascript(), foldGutter(), drawSelection(), dropCursor(), EditorState.allowMultipleSelections.of(true), indentOnInput(), defaultHighlightStyle.fallback, bracketMatching(), closeBrackets(), autocompletion(), rectangularSelection(), undefined, highlightSelectionMatches(), keymap.of([].concat(_toConsumableArray(closeBracketsKeymap), _toConsumableArray(defaultKeymap), _toConsumableArray(searchKeymap), _toConsumableArray(historyKeymap), _toConsumableArray(foldKeymap), _toConsumableArray(commentKeymap), _toConsumableArray(completionKeymap), _toConsumableArray(lintKeymap), [indentWithTab])), oneDark, theme].filter(function (f) {
  return f;
});
function Editor(parent) {
  ...
  return new EditorView({
    state: EditorState.create({
      extensions: [].concat(_toConsumableArray(setup), [EditorView.updateListener.of(function (vu) {
        if (vu.docChanged) {
          var doc = vu.state.doc.toString();
          console.log(doc);
        }
